...
describe("AuthGuard", () => {
  ...
  describe("requireRole", () => {
    it("should allow users with required role", async () => {
      ...
      expect(result.allowed).toBe(true);
      expect(mockPermissionManager.hasRole).toHaveBeenCalledWith(
        "user-123",
        "admin",
      );
    });
  ...
  describe("requirePermission", () => {
    it("should allow users with required permission", async () => {
      ...
      const result = await authGuard.requirePermission(
        "write",
        "document",
      )(context);
  ...
  describe("requireNSMClassification", () => {
    it("should allow access to lower or equal classification", async () => {
      ...
      const result2 =
        await authGuard.requireNSMClassification("RESTRICTED")(context);
    ...
    it("should block access to higher classification", async () => {
      ...
      const result =
        await authGuard.requireNSMClassification("CONFIDENTIAL")(context);

      expect(result.allowed).toBe(false);
      expect(result.reason).toBe(
        "Insufficient NSM classification: CONFIDENTIAL required, user has OPEN",
      );
      expect(result.statusCode).toBe(403);
    });
  });
