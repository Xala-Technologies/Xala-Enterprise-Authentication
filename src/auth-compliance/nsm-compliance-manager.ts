...
import { NSMClassificationLevels, validateNSMClassification } from '../utils/norwegian-compliance.js';
...
export class DefaultNSMComplianceManager implements NSMComplianceManager {
  ...
  private readonly classificationControls: Map<NSMClassification, Set<string>> = new Map();
  ...
  async checkCompliance(): Promise<boolean> {
    ...
    const requiredControls = Array.from(this.controls.values()).filter(c => c.required);
  ...
  async getComplianceReport(): Promise<ComplianceReport> {
    ...
    const missingControls = controls.filter(c => c.required && !c.implemented);
    ...
    const compliance: Record<string, boolean> = {
      ...
      vulnerabilityManagement: this.checkCategoryCompliance('vulnerability_management'),
  ...
  async checkClassificationCompliance(data: unknown, classification: NSMClassification): Promise<boolean> {
    // Get required controls for classification
    const requiredControlIds = this.classificationControls.get(classification) ?? new Set();
  ...
  async getRequiredControls(classification: NSMClassification): Promise<SecurityControl[]> {
    const controlIds = this.classificationControls.get(classification) ?? new Set();
  ...
  async assessRiskLevel(): Promise<'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'> {
    ...
    const implementationRate = implementedControls.length / requiredControls.length;
    const avgEffectiveness = this.calculateAverageEffectiveness(implementedControls);
...
`;
  ...
  private mapClassificationControls(): void {
    ...
    this.classificationControls.set('RESTRICTED', new Set([
      'ac-1', 'ac-2', 'ac-3', 'enc-2', 'mon-1', 'ir-1'
    ]));
    
    // CONFIDENTIAL - Most controls
    this.classificationControls.set('CONFIDENTIAL', new Set([
      'ac-1', 'ac-2', 'ac-3', 'enc-1', 'enc-2', 'enc-3',
      'mon-1', 'mon-2', 'ir-1', 'ir-2'
    ]));
  ...
  private getControlSeverity(control: SecurityControl): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    if (control.category === 'encryption' || control.category === 'access_control') {
      return 'CRITICAL';
    } else if (control.category === 'monitoring' || control.category === 'incident_response') {
      return 'HIGH';
    } else {
      return 'MEDIUM';
    }
  ...
  private calculateAverageEffectiveness(controls: SecurityControl[]): number {
    const effectiveControls = controls.filter(c => c.effectiveness !== undefined);
    if (effectiveControls.length === 0) return 0;
    
    const sum = effectiveControls.reduce((total, c) => total + (c.effectiveness ?? 0), 0);
    return Math.round(sum / effectiveControls.length);
  }

  private generateCategoryReport(category: SecurityControlCategory): string {
    const controls = Array.from(this.controls.values()).filter(c => c.category === category);
  ...
  private generateRecommendations(issues: ComplianceIssue[], controls: SecurityControl[]): ComplianceRecommendation[] {
    const recommendations = [];
    
    // Priority 1: Implement missing critical controls
    const missingCritical = controls.filter(c => 
      c.required && !c.implemented && 
  ...
  private generateSecurityRecommendations(): string {
    ...
    const missingControls = Array.from(this.controls.values()).filter(c => c.required && !c.implemented);
    if (missingControls.length > 0) {
      recommendations.push(`1. Implement missing controls: ${missingControls.map(c => c.name).join(', ')}`);
    }
    
    const lowEffectiveness = Array.from(this.controls.values()).filter(c => 
      c.implemented && c.effectiveness !== undefined && c.effectiveness < 70
    );
    if (lowEffectiveness.length > 0) {
      recommendations.push(`2. Improve effectiveness of: ${lowEffectiveness.map(c => c.name).join(', ')}`);
    }
  ...
  private checkSecretCompliance(data: unknown): boolean {
    ...
    return Array.from(this.controls.values()).every(c => !c.required || c.implemented);
  }

  private checkConfidentialCompliance(data: unknown): boolean {
    // CONFIDENTIAL requires encryption and strong access control
    const requiredCategories: SecurityControlCategory[] = ['encryption', 'access_control', 'monitoring'];
  ...
  private checkRestrictedCompliance(data: unknown): boolean {
    ...
    return this.checkCategoryCompliance('access_control') && 
