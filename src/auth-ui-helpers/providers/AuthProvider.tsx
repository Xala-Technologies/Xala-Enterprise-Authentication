...
export function AuthProvider({ children, config, onError, onSessionExpired }: AuthProviderProps) {
  ...
  const login = useCallback(async (credentials: LoginCredentials) => {
    ...
    try {
      ...
      if (config.persistSession) {
        localStorage.setItem('xala-auth-session', JSON.stringify({
          id: data.sessionId,
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    hasRole,
    sessionInfo,
  }), [user, isLoading, error, login, logout, refresh, checkPermission, hasRole, sessionInfo]);
